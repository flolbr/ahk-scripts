...
;; JetBrains IDEs
#if (getKeyState("F24", "P")) and (if WinActive("ahk_exe idea64.exe") or if WinActive("ahk_exe clion64.exe") or if WinActive("ahk_exe pycharm64.exe") or if WinActive("ahk_exe rubymine64.exe"))
; b::tooltip, build
b::Send, ^{F9} ; build
r::Send, +{F10} ; run
d::Send, +{F9} ; debug
...
#if (getKeyState("F24", "P")) and (if WinActive("ahk_exe emStudio.exe"))
; b::tooltip, build
; r::Send, +{F10} ; run
; d::Send, +{F9} ; debug
BackSpace::Send, ^+{F5} ; Restart
Space::Send, {F5} ; Pause/Resume
Down::Send, {F10} ; Step over
Right::Send, {F11} ; Step into
*::Send, {F9} ; Add/remove breakpoint

#if

;; Keil
#if (getKeyState("F24", "P")) and (if WinActive("ahk_exe UV4.exe"))
b::Send, {F7} ; build
l::Send, {F8} ; load
